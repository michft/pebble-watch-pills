...
function settingsSection(settings, appearance) {
  ...
  var zoneRows = zones.map(function (zone, index) {
    ...
    return "<div class='timezone configurable-row' id=zone-row-" + index
      ...
      + "-scheme-preview aria-hidden=true>12:34</div>"
      + "<label for=zone-" + index + "-use-digits>Time display</label>"
      + "<select id=zone-" + index + "-use-digits>"
      + options([[0, "Words"], [1, "Digits"]], zone.useDigits ? 1 : 0) + "</select></div>";
  }).join("");
  return "<section class=card><h2>Phone appearance</h2>"
    ...
    + "<p class=muted>Words and reminder times follow the watch's 12/24-hour system setting. Digits always use 24-hour time.</p>"
    ...
    + "<fieldset><legend>Watchface</legend>"
    + "<p class=muted>Digits show hours 0–23 on the top row and minutes 0–59 on the bottom row, without leading zeroes.</p>"
...
exports.buildReportPage = function buildReportPage(state) {
  ...
  return "<!doctype html><html lang=en data-appearance='" + appearance + "'><head><meta charset=utf-8>"
    ...
    + "function saveSettings(){var slots=[];for(var i=0;i<4;i++){var p=document.getElementById('slot-'+i+'-time').value.split(':');if(p.length!==2){alert('Set all reminder times.');return;}slots.push({id:i,hour:parseInt(p[0],10),minute:parseInt(p[1],10),enabled:document.getElementById('slot-'+i+'-enabled').checked});}for(var l=0;l<4;l++){if(!slots[l].enabled)continue;for(var r=l+1;r<4;r++){if(!slots[r].enabled)continue;var gap=Math.abs((slots[l].hour*60+slots[l].minute)-(slots[r].hour*60+slots[r].minute));gap=Math.min(gap,1440-gap);if(gap<2){alert('Enabled reminders need at least a two minute gap.');return;}}}var zones=[];for(var z=0;z<" + TIMEZONE_COUNT + ";z++){var label=document.getElementById('zone-'+z+'-label').value.trim().toUpperCase();if(!/^[A-Z0-9 ]{1,8}$/.test(label)){alert('Timezone labels need 1-8 letters, numbers, or spaces.');return;}var scheme=document.getElementById('zone-'+z+'-scheme').value.split(',');zones.push({id:z,enabled:z===0||document.getElementById('zone-'+z+'-enabled').checked,timeZone:document.getElementById('zone-'+z+'-time-zone').value,useDigits:document.getElementById('zone-'+z+'-use-digits').value==='1',label:label,textColor:parseInt(scheme[0],10),backgroundColor:parseInt(scheme[1],10)});}closeWith({action:'save_settings',appearance:document.getElementById('appearance').value,display:{useDigits:zones[0].useDigits,horizontal:parseInt(document.getElementById('horizontal').value,10),vertical:parseInt(document.getElementById('vertical').value,10),fontSize:parseInt(document.getElementById('font-size').value,10),textColor:zones[0].textColor,backgroundColor:zones[0].backgroundColor},zones:zones,slots:slots})}"
    + "function historySelection(){var value=document.getElementById('history-retention').value;var days=value==='all'?0:parseInt(value,10);if(days===0)return{days:days,events:historyEvents.slice()};var cutoff=Date.now()-days*24*60*60*1000;return{days:days,events:historyEvents.filter(function(event){return event.scheduledAt<=cutoff})}}"
